...
class Config {
    ...
    /**
     * Setzt die Guild-ID für diese Konfigurationsinstanz
     * @param {string|null} guildId - Guild-ID oder null für globale Konfiguration
     */
    setGuildId(guildId) {
     ...
     */
    async init(dbService, context = 'shared', guildId = null) {
        const Logger = ServiceManager.get('Logger');
    ...
    async get(context = this.defaultContext) {
        ...
        const cacheKey = `${this.pluginName}_${context}_${this.guildId || 'global'}`;
    ...
    async set(key, value, context = this.defaultContext) {
        ...
        try {
            ...
            await dbService.setConfig(
                ...
                context,
                this.guildId || null,
                !this.guildId // is_global = true wenn keine guildId
            );
            ...
            Logger.error(`Fehler beim Speichern der Konfiguration ${key}:`, error);
            return false;
        }
    }
    ...
    async setMultiple(configValues, context = this.defaultContext) {
        ...
        try {
            ...
            await connection.rollback();
            Logger.error(`Fehler beim Speichern mehrerer Konfigurationen:`, error);
            return false;
        } finally {
            connection.release();
        }
    ...
    async save(configToSave, context = this.defaultContext) {
        ...
        try {
            // Zuerst alle existierenden Configs für dieses Plugin/Context löschen
            await dbService.deleteConfig(
                this.pluginName,
                null, // configKey = null löscht alle Keys
                context,
                this.guildId || null
            );
