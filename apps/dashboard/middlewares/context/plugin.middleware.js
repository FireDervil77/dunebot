...
module.exports.dashboard = async (req, res, next) => {
    ...
    if (req.method === "PUT") {
        ...
        if (req.query.operation && req.query.operation === "prefix_commands_toggle") {
            try {
                ...
                const coreSettings = await pluginManager
                    .getPlugin("core")
                    .dbService.getSettings(guildId);

                coreSettings.disabled_prefix = Array.from(disabled);
                await dbService.query(`
                    INSERT INTO settings (_id, prefix, locale, enabled_plugins, disabled_prefix, disabled_slash)
                    VALUES (?, ?, ?, ?, ?, ?)
                    ON DUPLICATE KEY UPDATE
                        prefix = VALUES(prefix),
                        locale = VALUES(locale),
                        enabled_plugins = VALUES(enabled_plugins),
                        disabled_prefix = VALUES(disabled_prefix),
                        disabled_slash = VALUES(disabled_slash)
                `, [
                    req.params.guildId || req.context.guildId,
                    coreSettings.prefix || '!',
                    coreSettings.locale || 'de-DE',
                    coreSettings.enabled_plugins ? JSON.stringify(coreSettings.enabled_plugins) : JSON.stringify(['core']),
                    coreSettings.disabled_prefix ? JSON.stringify(coreSettings.disabled_prefix) : JSON.stringify([]),
                    coreSettings.disabled_slash ? JSON.stringify(coreSettings.disabled_slash) : JSON.stringify([])
                ]);
        ...
        if (req.query.operation && req.query.operation === "slash_commands_toggle") {
            try {
                ...
                const coreSettings = await pluginManager.getPlugin("core").dbService.getSettings(guildId);
                    
                // Diese Zeile wurde vergessen:
                coreSettings.disabled_slash = Array.from(disabled);
                
                const SettingsModel = pluginManager.getPlugin("core").dbService.getModel("Settings");
                await dbService.query(`
                    INSERT INTO settings (_id, prefix, locale, enabled_plugins, disabled_prefix, disabled_slash)
                    VALUES (?, ?, ?, ?, ?, ?)
                    ON DUPLICATE KEY UPDATE
    ...
    const [coreSettings, config] = await Promise.all([
        pluginManager.getPlugin("core").dbService.getSettings(guildId),plugin.getConfig()]);
