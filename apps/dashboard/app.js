...
const ThemeManager = require('./helpers/ThemeManager');
const ShortcodeParser = require("dunebot-sdk/lib/utils/ShortcodeParser");
const NotificationManager = require('./helpers/NotifcationManager');
const UpdatesManager = require('./helpers/UpdatesManager');
...
module.exports = class App {
    constructor(ipcServer, dbService) {
        ...
        this.app = express();
        this.app.set('trust proxy', 1); // Vertraue dem ersten Proxy
        
        // den Logger aus dem ServiceManager nutzten
        ...
        try {
            ...
            try {
                ...
                if (result.count === 0) {
                    ...
                    `);
                    Logger.info("Notifications-Tabelle erfolgreich erstellt");
                }
    ...
    async enableGuildSpecificPlugins() {
        ...
        try {
            // Alle Guild-Settings aus der DB holen
            const allSettings = await dbService.query(
                "SELECT _id, enabled_plugins FROM settings"
            );
            if (!allSettings || allSettings.length === 0) {
                Logger.warn("Keine Guild-Settings gefunden, keine Guild-spezifischen Plugins aktiviert");
                return;
            }
            ...
            // Für jede Guild die Plugins aktivieren
            for (const settings of allSettings) {
                if (!settings.enabled_plugins) continue;
                ...
                try {
                    ...
                    for (const pluginName of enabledPlugins) {
                        // Core Plugin überspringen
                        if (pluginName === 'core') continue;
                        
                        // Plugin global aktivieren, wenn noch nicht geschehen
                        ...
                        // Plugin für diese spezifische Guild aktivieren
                        try {
                    ...
                    Logger.error(`Fehler bei der Verarbeitung der Plugins für Guild ${settings._id}:`, err);
                }
            }
        } catch (error) {
    ...
    #initializeRoutes() {
        ...
        this.app.use("/guild", CheckAuth, guildRouter);
