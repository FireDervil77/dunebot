...
Guild.prototype.getT = function (key, args) {
    ...
    // Wenn keine Übersetzungsfunktion gefunden, nutze Fallback
    if (typeof tFunction !== 'function') {
        console.warn(`No translation function found for locale ${locale}, using fallback`);
        // Fallback zur Default-Sprache
        const fallbackFunction = this.client.i18n.translations.get(this.client.defaultLanguage);
        if (typeof fallbackFunction === 'function') {
            return fallbackFunction(key, args);
        }
        // Wenn auch das fehlschlägt, gib den Key zurück
        return key;
    }
...
Guild.prototype.getSettings = async function (pluginName) {
    ...
    if (!plugin || !plugin.dbService || typeof plugin.dbService.getConfig !== "function") {
        return null;
    }
    return plugin.dbService.getConfig(this.id);
};
