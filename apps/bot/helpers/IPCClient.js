...
class IPCClient {
        ...
        return new veza.Client(`Bot #${this.shardId}`, {
            ...
            .on("error", (error, client) => {
                ...
                if (error.code === 'ECONNRESET') {
                    Logger.warn(`[IPC] Verbindung zurückgesetzt von ${client?.name || 'unbekannt'}, versuche neu zu verbinden...`);
                    // Wiederverbindung nach kurzer Verzögerung versuchen
                    setTimeout(() => this.reconnect(), 2000);
                    return;
                }
                Logger.error(`[IPC] Fehler von ${client?.name || 'unbekannt'}:`, error);
            })
            .on("disconnect", (client) => {
                Logger.warn(`[IPC] Verbindung getrennt von ${client?.name || 'unbekannt'}, versuche neu zu verbinden...`);
                // Wiederverbindung nach kurzer Verzögerung versuchen
                setTimeout(() => this.reconnect(), 2000);
            })
            .on("ready", async (_client) => {
                Logger.success(`[IPC] Bot-Client (Shard#${this.shardId}) erfolgreich mit Dashboard-IPC-Server verbunden`);
    ...
    async reconnect() {
        try {
            Logger.info(`[IPC] Versuche, erneut eine Verbindung zum IPC-Server herzustellen: ${this.host}:${this.port}`);
            
            // Prüfen, ob der Port erreichbar ist
            const isPortAvailable = await this.isPortReachable(this.port, this.host);
            if (!isPortAvailable) {
                Logger.warn(`[IPC] Port ${this.port} auf Host ${this.host} scheint nicht erreichbar zu sein`);
            }
            
            await this.connect();
        } catch (error) {
            Logger.error("[IPC] Fehler bei der Wiederverbindung:", error);
        }
    }
    ...
    async connect() {
        try {
            ...
            if (!isReachable) {
                Logger.warn(`[IPC] Port ${this.port} auf Host ${this.host} ist nicht erreichbar. IPC-Server läuft vermutlich nicht.`);
            ...
            Logger.debug(`[IPC] Port ${this.port} auf Host ${this.host} ist erreichbar. Verbindungsversuch wird gestartet...`);
            ...
            return this.node
                ...
                .then(() => {
                    this.firstConnect = true;
                    Logger.info("[IPC] Verbindung hergestellt");
                })
                .catch((error) => {
                    if (error.code == "ECONNREFUSED") {
                        Logger.warn("[IPC] Verbindung verweigert - Server möglicherweise nicht gestartet oder überlastet");
                        return;
                    }
                    Logger.error("[IPC] Verbindungsfehler:", error);
                });
        } catch (error) {
            Logger.error("[IPC] Fehler beim Verbindungsversuch:", error);
        }
    }
    ...
    initialize(client) {
        ...
        if (this._initialized) {
            Logger.debug("[IPC] IPC-Client wurde bereits initialisiert, überspringe...");
            return;
        }
        
        Logger.info("[IPC] Initialisiere IPC-Client...");
    ...
    async handleMessage(message) {
        ...
        Logger.debug(`[IPC] Bot empfängt IPC-Event: ${message?.data?.event || "unbekannt"}`);
    ...
    async #handleBaseMessage(eventName, message) {
        Logger.debug(`[IPC] Verarbeite Dashboard-Event: ${eventName}`);
        ...
        try {
            switch (eventName) {
                ...
                default:
                    Logger.warn(`[IPC] Unbekanntes Dashboard-Event: ${eventName}`);
                    return message.reply({
                        success: false,
            ...
            Logger.error(`[IPC] Fehler bei der Verarbeitung von Dashboard-Event ${eventName}:`, error);
            return message.reply({
                success: false,
    ...
    async #handleGetGuildInfo(message, payload) {
        try {
            ...
            Logger.error("[IPC] Fehler beim Abrufen der Guild-Informationen:", error);
            return message.reply({
                success: false,
                error: error.message
            });
    ...
    async #handleGetStats(message) {
        try {
            ...
            Logger.error("[IPC] Fehler beim Abrufen der Bot-Statistiken:", error);
            return message.reply({
                success: false,
                error: error.message
            });
    ...
    async #handleGetGuildStats(message, payload) {
        try {
                    ...
                    if (corePlugin && corePlugin.dbService) {
                        ...
                        if (settings && settings.enabled_plugins) {
                            try {
                                ...
                                Logger.warn(`[IPC] Fehler beim Parsen der aktivierten Plugins für Guild ${guild.id}:`, e);
                                enabledPlugins = ['core']; // Fallback
                            }
                        }
                    }
                } catch (error) {
                    Logger.error(`[IPC] Fehler beim Ermitteln der aktivierten Plugins für Guild ${guild.id}:`, error);
                }
            }
            ...
            Logger.error("[IPC] Fehler beim Abrufen der Guild-Statistiken:", error);
            return message.reply({
                success: false,
                error: error.message
            });
    ...
    async #handleValidateGuild(message, payload) {
        try {
            ...
            Logger.error("[IPC] Fehler bei der Guild-Validierung:", error);
            return message.reply({
                success: false,
                error: error.message
            });
    ...
    async #handleGetBotGuilds(message) {
        try {
            ...
            Logger.error("[IPC] Fehler beim Abrufen der Bot-Guilds:", error);
            return message.reply({
                success: false,
                error: error.message
            });
    ...
    async #handleGetCommandsSummary(message, payload) {
        try {
            ...
            Logger.error("[IPC] Fehler beim Abrufen der Befehls-Zusammenfassung:", error);
            return message.reply({
                success: false,
                error: error.message
            });
    ...
    async #handleGetPluginCommands(message, payload) {
        try {
            ...
            Logger.error("[IPC] Fehler beim Abrufen der Plugin-Befehle:", error);
            return message.reply({
                success: false,
                error: error.message
            });
    ...
    async #handleGetGuildExtendedStats(message, payload) {
        try {
           ...
           // Rollen-Details mit Berechtigungen
            ...
            Logger.error("[IPC] Fehler beim Abrufen der erweiterten Guild-Statistiken:", error);
            return message.reply({
                success: false,
                error: error.message
            });
    ...
    async #handleGetLocaleBundle(message, payload) {
        try {
            ...
            Logger.error("[IPC] Fehler beim Abrufen des Lokalisierungs-Bundles:", error);
            return message.reply({
                success: false,
                error: error.message
            });
    ...
    async #handleSetLocaleBundle(message, payload) {
        try {
            ...
            Logger.error("[IPC] Fehler beim Setzen des Lokalisierungs-Bundles:", error);
            return message.reply({
                success: false,
                error: error.message
            });
