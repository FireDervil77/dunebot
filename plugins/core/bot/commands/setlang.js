...
async function setNewLang(guild, newLang) {
    ...
    const SettingsModel = dbService.getModel("Settings");
    const enabledPluginsEncoded = typeof settings.enabled_plugins === 'string'
        ? settings.enabled_plugins
        : JSON.stringify(settings.enabled_plugins || ["core"]);

    await SettingsModel.upsert({
        _id: guild.id,                  // FIX: kein message.*, sondern guild.id
        ...settings,
        enabled_plugins: enabledPluginsEncoded
    });

    const t = guild.getT ? guild.getT.bind(guild) : null;
    return {
        success: true,
        // Statt "LANGUAGE_CHANGED" jetzt "LANG.SUCCESS"
        content: t ? t("LANG.SUCCESS", { lang: newLang }) : `Erfolg! Die Sprache wurde auf ${newLang} gesetzt`
    };
}