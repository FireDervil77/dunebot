...
async function listPlugins({ client, guild }) {
    ...
    const found = await SettingsModel.findOne({ where: { _id: guild.id } });
    const enabled_plugins = found?.enabled_plugins || ["core"];
...
async function pluginInfo({ client, guild }, plugin) {
    ...
    const { enabled_plugins } = await dbService.getSettings(guild);
    const embed = EmbedUtils.embed()
...
async function pluginStatus(arg0) {
    ...
    const SettingsModel = dbService.getModel("Settings");
    await SettingsModel.upsert({ _id: guild.id, ...settings });
