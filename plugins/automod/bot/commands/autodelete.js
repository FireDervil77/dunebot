...
async function antiAttachments(settings, input, guild) {
    ...
    await AutoModSettings.updateSettings(guild.id, { anti_attachments: status });
    return status
...
async function antiInvites(settings, input, guild) {
    const status = input.toUpperCase() === "ON" ? true : false;
    await AutoModSettings.updateSettings(guild.id, { anti_invites: status });
    return status
...
async function antilinks(settings, input, guild) {
    const status = input.toUpperCase() === "ON" ? true : false;
    await AutoModSettings.updateSettings(guild.id, { anti_links: status });
    return status
...
async function maxLines(settings, input, guild) {
    ...
    await AutoModSettings.updateSettings(guild.id, { max_lines: lines });
    return input === 0
