...
async function antiGhostPing(settings, input, guild) {
    ...
    await AutoModSettings.updateSettings(guild.id, { anti_ghostping: status });
    return status
...
async function antiSpam(settings, input, guild) {
    const status = input.toUpperCase() === "ON" ? true : false;
    await AutoModSettings.updateSettings(guild.id, { anti_spam: status });
    return status
...
async function antiMassMention(settings, input, threshold, guild) {
    ...
    await AutoModSettings.updateSettings(guild.id, { 
        anti_massmention: status,
        anti_massmention_threshold: threshold 
    });
    return status
